...
class Ainsys_Init {
	...
	public static function add_ansys_admin_menu() {
		add_menu_page(
			...
			array( __CLASS__, 'include_setting_page' ),
			'dashicons-randomize',
			10
		);
	}
	...
	public static function admin_enqueue_scripts() {
		wp_enqueue_script( 'ainsys_connector_admin_handle', plugins_url( 'js/ainsys_connector_admin.js', AINSYS_CONNECTOR_PLUGIN ), array( 'jquery' ), '2.0.0', true );

		if ( isset( $_GET['page'] ) && self::$settings_page_name === $_GET['page'] ) {
			//wp_enqueue_script('jquery-ui-sortable');
			wp_enqueue_style( 'ainsys_connector_style_handle', plugins_url( 'css/ainsys_connector_style.css', AINSYS_CONNECTOR_PLUGIN ) );
