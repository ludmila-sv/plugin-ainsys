...
class Ainsys_Settings {
	...
	static function init() {
		...
		$limit_date   = (int) Ainsys_Settings::get_option( 'log_until_certain_time' ) ? gmdate( 'Y-m-d H:i:s', Ainsys_Settings::get_option( 'log_until_certain_time' ) ) : '';

		if ( ( ! (int) Ainsys_Settings::get_option( 'log_until_certain_time' ) || $currant_date < $limit_date )
	...
	public static function generate_hook_url() {
		return site_url( '/?ainsys_webhook=' . ainsys_webhook_listener::$request_token, 'https' );
	}
